"""
card_generator_improved.py — Enhanced Python/Pillow card generator
...
def _find_font(bold: bool = False, mono: bool = False) -> Optional[str]:
    """Return path to an available TTF font, or None."""
    candidates = [
        os.path.join(_HERE, "LobsterTwo-Regular.ttf"),
        os.path.join(_HERE, "Monoton-Regular.ttf"),
    ]
...
# ----------------------------------------------------------------------
# Configuration – Optimized Design System
# ----------------------------------------------------------------------
@dataclass
class CardConfig:
    width: int = 960
    height: int = 540  # Slightly taller for better proportions
    
    # Padding / margins (percent)
    padding_h: float = 0.06
    padding_v: float = 0.06
    avatar_size: float = 0.13  # Slightly smaller avatar
    divider_y: float = 0.38
    right_panel_width: float = 0.28
    
    # Modern color palette - More refined
    bg_dark: Tuple[int, int, int] = (15, 23, 42)      # Slate 900
    bg_card: Tuple[int, int, int] = (30, 41, 59)       # Slate 800
    accent_credit: Tuple[int, int, int] = (16, 185, 129)   # Emerald 500
    accent_debit: Tuple[int, int, int] = (239, 68, 68)     # Red 500
    surface_light: Tuple[int, int, int] = (51, 65, 85)     # Slate 700
    surface_dark: Tuple[int, int, int] = (30, 41, 59)      # Slate 800
    text_primary: Tuple[int, int, int] = (248, 250, 252)   # Slate 50
    text_muted: Tuple[int, int, int] = (148, 163, 184)     # Slate 400
    divider_color: Tuple[int, int, int] = (71, 85, 105)    # Slate 600
    
    # Font sizes (relative to height) - Better hierarchy
...
# ----------------------------------------------------------------------
# Main generation – Improved Layout & Design
# ----------------------------------------------------------------------
def generate_card(user_id: str, display_name: str, pending: float, date: str,
    ...
    abs_str = f"{abs_pend:,.2f}"  # Add thousand separators
    ...
    # ================================================================
    # 1. IMPROVED Gradient Background - Smoother, more subtle
    # ================================================================
    for y in range(H):
        t = y / H
    ...
    # ================================================================
    # 2. Refined Glow Orb - Softer, better positioned
    # ================================================================
    ...
    for r in range(rad, 0, -3):
        alpha_val = 0.12 * (1 - r / rad) ** 2  # Quadratic falloff
    ...
    # ================================================================
    # 3. Subtle Grid Pattern - Less prominent
    # ================================================================
    ...
    # ================================================================
    # 4. Card Border - Thinner, more elegant
    # ================================================================
    border_alpha = 0.18
    ...
    # ================================================================
    # 5. Top Accent Stripe - More subtle gradient
    # ================================================================
    stripe_height = 3
    ...
    # ================================================================
    # 6. Avatar - Improved glow and styling
    # ================================================================
    av_x, av_y = pad_x, int(pad_y * 1.2)
    
    # Outer glow - softer, fewer rings
    ...
    # Avatar background - slightly transparent
    draw.rounded_rectangle(
    ...
    # Initial letter - better centered
    ...
    # ================================================================
    # 7. Name and ID - Better spacing and alignment
    # ================================================================
    f_name = _font(bold=True, size=fs(config.font_name))
    name_y = av_y + int(H * 0.01)
    draw.text((name_x, name_y), display_name,
              font=f_name, fill=config.text_primary)
    
    f_id = _font(size=fs(config.font_id))
    id_y = name_y + fs(config.font_name) + int(H * 0.015)
    draw.text((name_x, id_y), user_id.upper(),
              font=f_id, fill=config.text_muted)

    # ================================================================
    # 8. Logo - Repositioned and styled
    # ================================================================
    ...
    draw.text((W - pad_x - logo_w, logo_y),
              logo_text, font=f_logo, 
              fill=_alpha_blend(accent, 0.65, config.bg_card))

    # ================================================================
    # 9. Divider Line - Subtle gradient
    # ================================================================
    for x in range(pad_x, W - pad_x):
        t = (x - pad_x) / (W - 2*pad_x)
        div_alpha = 0.4 + 0.3 * (1 - abs(t - 0.5) * 2)  # Brighter in center
        div_col = _alpha_blend(config.divider_color, div_alpha, config.bg_card)
        draw.point((x, divider_y), fill=div_col)

    # ================================================================
    # 10. Balance Section - Improved layout
    # ================================================================
    bal_label_y = divider_y + int(H * 0.055)
    
    f_label = _font(bold=True, size=fs(config.font_balance_label))
    draw.text((pad_x, bal_label_y), bal_label, 
              font=f_label, fill=config.text_muted)

    # Symbol and Amount - Better vertical alignment
    ...
    draw.text((amt_x, baseline_y - amount_h), abs_str,
              font=f_amount, fill=accent)

    # ================================================================
    # 11. Status Pill - Refined design
    # ================================================================
    pill_h = int(H * 0.07)
    ...
    draw.text((status_text_x, status_text_y), status_lbl,
              font=f_status, fill=accent)

    # ================================================================
    # 12. Date - Better positioned
    # ================================================================
    ...
    # ================================================================
    # 13. Right Stats Panel - Redesigned
    # ================================================================
    panel_y = pad_y
    panel_h = H - 2 * pad_y
    
    # Panel background - subtle gradient effect
    draw.rounded_rectangle(
    ...
    # ✅ CORRECT - Both coordinates in one tuple
    ...
    # Mini bar chart - improved styling
    ...
    for i, bv in enumerate(bars):
        ...
        # Bar color - last one highlighted
        if i == len(bars) - 1:
            color = accent
            # Glow on last bar
        ...
        draw.rounded_rectangle([bx, by, bx + bar_w, by + bh], 
                              radius=3, fill=color)

    # Stat rows - cleaner layout
    ...
    for idx, (label, value) in enumerate(stat_data):
        ...
        draw.text((right_panel_x + chart_pad, row_y), label,
                  font=f_stat_label, fill=_alpha_blend(config.text_muted, 0.8, config.surface_light))
        ...
        draw.text((right_panel_x + chart_pad, val_y), value,
                  font=f_stat_value, fill=_alpha_blend(value_color, 0.9, config.surface_light))
...
if __name__ == "__main__":
    # Generate test cards
    print("Generating credit card...")
    credit_data = generate_card(
        user_id="U123456",
        display_name="John Doe",
        pending=-1250.75,  # Negative = credit
        date="2026-04-09"
    )
    with open("card_credit.png", "wb") as f:
        f.write(credit_data)
    print("✓ Credit card saved: card_credit.png")

    print("\nGenerating debit card...")
    debit_data = generate_card(
        user_id="U789012",
        display_name="Jane Smith",
        pending=850.50,  # Positive = debit
        date="2026-04-09"
    )
    with open("card_debit.png", "wb") as f:
        f.write(debit_data)
    print("✓ Debit card saved: card_debit.png")